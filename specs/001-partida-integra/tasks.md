...
- [X] T026 [US1] Reglas de `quizzes/{q}`, `quizzes/{q}/questions/{n}` y `quizzes/{q}/solutions/{n}` en `firestore.rules`: lectura pública de las dos primeras, lectura **solo del presentador** de las soluciones, escritura solo del presentador, `delete` nunca, y `keys().hasOnly(['index','text','options','timeLimitSec'])` en el documento público
- [X] T027 [P] [US1] Test en `tests/rules/quiz-read.spec.ts`: denegaciones 1, 10 y 11 —un participante no puede leer `solutions/{n}`, no puede crear un cuestionario, y no se puede escribir `correctIndex` ni `teachingNote` dentro de `questions/{n}`— más la autorización 8, el presentador sí lee `solutions`
- [X] T028 [US1] Reglas de creación de ronda y del puntero `config/activeRound` en `firestore.rules`, atadas entre sí con `getAfter()`: crear una ronda activa exige que el puntero la señale, mover el puntero exige que la ronda quede activa, y si había ronda previa la transacción debe archivarla
- [X] T029 [P] [US1] Test en `tests/rules/active-round.spec.ts`: denegaciones 23 y 24 —crear ronda sin mover el puntero, y crear ronda nueva sin archivar la anterior— más la autorización 11
- [X] T030 [US1] Reglas de transición de fase sobre `rounds/{r}` en `firestore.rules` usando `diff().affectedKeys().hasOnly()`, con la tabla completa del contrato, la condición `currentIndex == resource.data.currentIndex + 1` en `revealed → open`, y `cualquier fase → archived`
- [X] T031 [P] [US1] Test en `tests/rules/transitions.spec.ts`: denegaciones 8 y 12 —conducir sin ser el presentador, retroceder de fase o repetir índice— más la autorización 5, y el rechazo de la transición a la misma fase que es el doble clic
- [X] T032 [US1] Reglas de la transacción de entrada en `firestore.rules`: `participants/{uid}`, `nicknames/{nick}` y el incremento de `participantCount`, con `getAfter(NK).data.uid == request.auth.uid` para la reserva propia y `!exists(P) && existsAfter(P)` en las otras dos
- [X] T033 [P] [US1] Test en `tests/rules/join-deny.spec.ts`: denegaciones 15 a 22. **Por cada condición cruzada, dos tests**: el documento ausente y el documento ya presente de antes. La 20 —entrar con la reserva de otro— y la 21 —incrementar el contador ya estando dentro— son las que sobrevivieron al primer diseño
- [X] T034 [P] [US1] Test en `tests/rules/join-allow.spec.ts`: autorizaciones 1 y 10 —una entrada completa se acepta, y dos participantes con apodos distintos entran de forma concurrente y ambos quedan contados—, más la 9, el presentador ajusta `maxParticipants` en curso (FR-012, SC-010)
- [X] T035 [US1] Reglas de `rounds/{r}/answers/{uid}_{n}` en `firestore.rules` con las diez condiciones del contrato, y **sin `update` ni `delete`**, que es lo que hace cumplir FR-028 sin lógica adicional
- [X] T036 [P] [US1] Test en `tests/rules/answer-deny.spec.ts`: denegaciones 4 a 7 —payload con campos de puntaje, respuesta tras el plazo, respuesta con la pregunta no abierta, segunda respuesta a la misma pregunta
- [X] T037 [P] [US1] Test en `tests/rules/answer-allow.spec.ts`: autorización 2, y que `optionIndex` fuera de rango o de tipo no entero se rechaza
- [X] T038 [US1] Reglas de `rounds/{r}/results/{n}`, `rounds/{r}/scores/{uid}` y `rounds/{r}/podium` en `firestore.rules`: agregados y podio de lectura pública, puntajes legibles solo por su dueño o el presentador, escritura de puntajes solo del presentador, `results` solo `create`
- [X] T039 [P] [US1] Test en `tests/rules/score-deny.spec.ts`: denegaciones 2, 3, 9 y 13 —leer la respuesta y el puntaje de otro, escribir el propio puntaje, revelar dos veces la misma pregunta
- [X] T040 [P] [US1] Test en `tests/rules/score-allow.spec.ts`: autorizaciones 3, 4, 6 y 7 —el participante lee lo suyo y los agregados, el presentador lee todas las respuestas de la pregunta y escribe agregados y puntajes

### Dominio de la historia

- [X] T041 [P] [US1] Implementar `src/domain/scoring.ts`: `BASE_POINTS = 100`, `MAX_SPEED_BONUS = 100`, `pointsFor(correct, elapsedMs, timeLimitSec)` y `totalsFor(perQuestion, timeLimits)`
- [X] T042 [P] [US1] Test en `tests/domain/scoring.test.ts`: respuesta instantánea da el bono completo; en el último instante da bono cero y **nunca negativo**; incorrecta da cero aunque sea instantánea; no respondida aporta cero puntos y **el tiempo límite completo** al desempate, porque FR-040 exige que abstenerse no favorezca (FR-037, FR-038, FR-039)
- [X] T043 [P] [US1] Implementar `src/domain/aggregate.ts`: `distributionOf(answers, optionCount)` contando sobre respuestas recibidas y no sobre participantes presentes, y `correctPctOf(answers, correctIndex)`
- [X] T044 [P] [US1] Test en `tests/domain/aggregate.test.ts`: con cero respuestas `correctPctOf` devuelve **0 y nunca `NaN`** (FR-078); con una sola respuesta la distribución la cuenta como 100% del total de respuestas (FR-044)
- [X] T045 [P] [US1] Implementar `src/domain/podium.ts`: `rankParticipants()` ordenando por `totalPoints` descendente y, ante empate, por `totalElapsedMs` ascendente, con posición compartida si el empate persiste; y `topThree()`
- [X] T046 [P] [US1] Test en `tests/domain/podium.test.ts`: empate resuelto por tiempo; empate exacto que comparte `rank` y hace saltar la siguiente posición; cero, uno y dos participantes; todos con cero puntos (FR-048)

### Capa de datos

- [X] T047 [US1] Implementar `src/data/rounds.ts`: suscripción al documento de ronda, y las acciones de conducción como transacciones de compare-and-swap sobre `(fase origen, fase destino)`
...
- [X] T052 [US1] Calibrar el retroceso exponencial de la entrada en `src/data/rounds.ts` con el resultado de T051, y registrar la medición en el riesgo abierto 2 de `research.md`. **Si el percentil 95 supera los 15 segundos**, detener e informar: la alternativa es hacer el contador aproximado y apoyarse solo en FR-012, y esa decisión se toma con la medición delante, como dice D4
...
- [X] T055 [US1] Implementar el flujo de calificación en `src/data/scores.ts`: al revelar, leer las respuestas de la pregunta y el documento de solución, calcular con `scoring.ts` y `aggregate.ts`, y escribir el agregado público más el puntaje de cada participante
...
- [X] T075 [P] [US3] Implementar `src/domain/quizFile.ts`: `parseQuizFile(raw)` que **acumula todos los errores** en vez de detenerse en el primero, y devuelve `{path, message}` con ubicación
- [X] T076 [P] [US3] Implementar las validaciones de `contracts/quiz-file.md` en `src/domain/quizFile.ts`: `title` string no vacío; `questions` array de al menos 1; `options` array de 2 a 6 strings no vacíos y sin repetidos; `correctIndex` entero con `0 <= correctIndex < options.length`; `teachingNote` requerida y no vacía; `timeLimitSec` entero entre 5 y 300 con 30 por defecto
...
- [X] T094 [P] Evaluar App Check con el proveedor reCAPTCHA v3, que es el gratuito en el plan Spark. Activarlo si la medición de T095 o una prueba en sala lo aconsejan, y **obligatoriamente si el uso deja de ser presencial** —sesiones remotas, enlace distribuido fuera de la sala, o participantes que no se ven entre sí—, porque ahí decae la baja probabilidad con la que D4 acepta la amenaza. Resolver los tokens de depuración para que no bloquee la suite de reglas en local
- [X] T095 Medir el presupuesto real de SC-006 según el procedimiento de `specs/001-partida-integra/quickstart.md`: correr una ronda completa contra el emulador, contar lecturas y escrituras en su UI, extrapolar a 50 participantes y comparar con los límites diarios del plan Spark
- [X] T096 Actualizar la estimación de D8 en `research.md` con la medición de T095, sustituyendo las cifras estimadas por las reales
- [X] T097 **Medir la latencia de revelación** con un script en `scripts/measure-reveal-fanout.ts`: abrir 50 suscripciones simultáneas al documento de ronda contra el emulador, revelar una pregunta y medir el retardo hasta que la última recibe el agregado. SC-002 exige menos de 2 segundos y ninguna tarea lo verificaba
...
- [X] T103 CRITICAL — Quitar `update` de `quizzes/{q}`, `quizzes/{q}/questions/{n}` y `quizzes/{q}/solutions/{n}` en `firestore.rules`, dejándolos solo `create`, con sus tests de denegación en `tests/rules/quiz-read.spec.ts`, para que una ronda en curso termine con el cuestionario con el que empezó por garantía del servidor y no solo porque `publishQuiz` genera un id nuevo; actualizar la matriz de `contracts/firestore-rules.md` per Constitution I, spec: Edge Case "El presentador publica un cuestionario con una ronda en curso" (contradicts)
- [X] T104 CRITICAL — Hacer idempotente `publishRound` en `src/data/rounds.ts` frente a un doble envío: hoy cada llamada crea una ronda nueva, así que un doble clic crea dos rondas y archiva la primera, y solo lo evita el botón deshabilitado de `PublicarRonda.tsx`. Usar un id de ronda generado una vez por formulario y terminar como `stale` si ya existe per Constitution V, FR-018 (partial)
- [X] T105 CRITICAL — Hacer idempotente `publishQuiz` en `src/data/quizzes.ts` frente a un doble envío: hoy cada llamada crea un cuestionario duplicado. Usar un id generado una vez por archivo cargado en `PublicarQuiz.tsx` y tratar como ya publicado el rechazo por documento existente (compatible con T103) per Constitution V, FR-018 (partial)
- [X] T106 Añadir a `tests/rules/idempotency.spec.ts` la aplicación doble de `publishQuiz`, `publishRound` y `adjustMaxParticipants`, las tres acciones del presentador sin ese test, y hacer que `adjustMaxParticipants` con el mismo valor termine sin cambios en vez de propagar el `permission-denied` que hoy muestra la interfaz per Constitution V ("para cada acción del presentador existe un test que la aplica dos veces") (partial)
- [X] T107 Ofrecer la reproyección de una pregunta ya revelada desde `src/ui/presenter/Conduccion.tsx` durante la partida en vivo, no solo en el podio: hoy solo se alcanza desde el podio o desde «Rondas pasadas» per FR-024, spec: Clarifications "Puede reproyectar la revelación de cualquier pregunta ya revelada para comentarla" (partial)

---

## Phase 9: Convergence

- [X] T108 CRITICAL — Enmendar el Principio II de `.specify/memory/constitution.md` con `/speckit-constitution` para reconciliarlo con FR-057 y con el supuesto "Autoridad de calificación" del spec. Hoy exige que las reglas denieguen la lectura de la respuesta de un `uid` ajeno "a cualquier participante, incluido el presentador", pero sin backend el cliente autenticado del presentador califica y `firestore.rules` le autoriza a leer respuestas y puntajes individuales. La enmienda debe conservar la denegación para todo otro participante, que ya prueban las denegaciones 2 y 3. Debe declarar que la garantía frente al presentador es la ausencia de PII y que ninguna vista suya muestre resultados individuales, verificada mecánicamente por `tests/domain/presenter-surface.test.ts`. Y debe subir la versión de la constitución per Constitution II (contradicts)
